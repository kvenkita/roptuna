---
title: "Getting Started with roptuna"
output:
  rmarkdown::html_document:
    ...
    theme: flatly
vignette: >
  %\VignetteIndexEntry{Getting Started with roptuna}
...
`roptuna` is an R port of the Python [Optuna](https://optuna.org/) framework
(Akiba et al. 2019). It automates hyperparameter search using a define-by-run
API: instead of defining a search space upfront, you call `trial$suggest_*()`
inside your objective function, and the sampler adapts based on what has
worked before.
...
- `trial$suggest_float(name, low, high, log = FALSE)` — continuous parameter
...
Parameters are named and idempotent: calling `suggest_float("lr", 0, 1)` twice
in the same trial always returns the same value.
...
intermediate values, check `trial$should_prune()`, and call `stop_prune()` to
signal that the trial should be abandoned:
