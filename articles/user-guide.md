...
`roptuna` implements `autoplot.Study` with three plot types. All three
accept the `Study` object returned by
[`create_study()`](https://kvenkita.github.io/roptuna/reference/create_study.md).
...
By default, `roptuna` stores trial results in memory and they are lost
when the R session ends. SQLite persistence provides a durable,
queryable record that survives session restarts and can be shared across
machines.
